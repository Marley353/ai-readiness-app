...
export class BaseScene implements Scene {
  ...
  private facilities(x: number, y: number, w: number, h: number) {
    ...
    const g = new Container(); p.content.addChild(g); const overlay = new Container(); const cells = this.buildPick ? validCells(b, this.buildPick) : [];
    for (let gy = 0; gy < 6; gy++) for (let gx = 0; gx < 6; gx++) {
      ...
      if (isOrigin) { const key = f.daysLeft > 0 ? 'facility/construction' : `facility/${f.def}`; const sz = (FACILITIES[f.def]?.size ?? 1) * cell - 8; const top = new Container(); top.position.set(gx * cell, gy * cell); top.eventMode = 'none'; if (has(key)) { const sp = sprite(key); sp.width = sz; sp.height = sz; sp.position.set(4, 4); top.addChild(sp); } const t = label(FACILITIES[f.def]?.name ?? f.def, { size: 'caption', weight: '600', wrap: sz }); t.position.set(6, 6); top.addChild(t); if (f.daysLeft > 0) { const d = readout(`${f.daysLeft} d`, { size: 'caption', color: P.warn }); d.position.set(6, sz - 14); top.addChild(d); } overlay.addChild(top); }
