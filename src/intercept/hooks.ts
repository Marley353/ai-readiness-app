...
export function installInterceptHooks() {
  ...
  hooks.add('intercept', {
    start: (craftId: number, ufoId: number) => { const s = getState(); const c = s.craft.find((x) => x.id === craftId); const u = s.ufos.find((x) => x.id === ufoId); if (!c || !u) return false; current = engagementForCraft(craftId) ?? startEngagement(s, craftId, ufoId, { craft: CRAFT[c.type], weapons: c.weapons.map((w) => (w ? CRAFT_WEAPONS[w.def] : null)), ufo: UFOS[u.type] }); return !!current; },
