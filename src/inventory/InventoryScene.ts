...
export class InventoryScene implements Scene {
  ...
  private items(): Placed[] {
    ...
    const craft = s.craft.find((c) => c.id === so.craftId); const pool = craft ? craft.items : (s.bases.find((b) => b.id === so.baseId)?.items ?? {}); let i = 0; const ground: Placed[] = []; for (const [id, n] of Object.entries(pool)) { if (n <= 0 || !ITEMS[id] || !['weapon', 'ammo', 'grenade', 'equipment'].includes(ITEMS[id].category)) continue; ground.push({ key: 1000 + i, def: id, slot: 'ground', gx: (i % 6) * 2, gy: Math.floor(i / 6) * 3, rounds: n }); i++; }
    return [...eq, ...ground];
  }
  private build() {
    ...
    const body = unit?.unitSprite ?? (so?.armour ? ITEMS[so.armour].armour?.unitSprite : 'xcom-none') ?? 'xcom-none'; if (has(`unit/${body}/4/idle`)) { const sp = sprite(`unit/${body}/4/idle`); sp.scale.set(2); sp.position.set(148, 72); layer.addChild(sp); }
  ...
  private tapSlot(slot: InvSlot, gx: number, gy: number) {
    ...
    const so = s.soldiers.find((x) => x.id === this.soldierId)!; const craft = s.craft.find((c) => c.id === so.craftId); const pool = craft ? craft.items : s.bases.find((b) => b.id === so.baseId)!.items;
