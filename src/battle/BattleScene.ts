...
export class BattleScene implements Scene {
  ...
  private buildHud() {
    ...
    const w = app.w; const H = 120; const y = app.h - app.safe.bottom - H; const p = new Panel(w, H + app.safe.bottom, { pad: S.x1 }); p.position.set(0, y); this.hud.addChild(p); const c = p.content; const E: any = this.hudEls = {};
    ...
    const et = button({ label: 'END TURN', icon: 'end-turn', w: 128, h: 48, variant: 'primary', onTap: () => this.endTurn() }); et.position.set(w - app.safe.right - S.x2 - 128, 24); c.addChild(et);
    const ab = button({ icon: 'abort', label: 'ABORT', w: 96, h: 48, variant: 'danger', size: 'caption', onTap: () => this.abort() }); ab.position.set(w - app.safe.right - S.x2 - 128 - S.x1 - 96, 24); c.addChild(ab);
    const opt = button({ icon: 'options', w: 48, h: 48, variant: 'ghost', onTap: () => scenes.show('options') }); opt.position.set(w - app.safe.right - S.x2 - 128 - S.x1 - 96 - S.x1 - 48, 24); c.addChild(opt);
    E.turn = readout('', { size: 'caption', color: P.textMuted }); E.turn.position.set(w - app.safe.right - S.x2 - 288, 80); c.addChild(E.turn);
