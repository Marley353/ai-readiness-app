...
export async function run() {
  const file = 'tools/critics/vitest-report.json'; await rm(file, { force: true });
  const out = await new Promise((res) => { const p = spawn('npx', ['vitest', 'run', '--reporter=json', `--outputFile=${file}`], { cwd: process.cwd() }); let s = ''; p.stdout.on('data', (d) => (s += d)); p.stderr.on('data', (d) => (s += d)); p.on('close', () => res(s)); });
  const defects = [];
  let json = null;
  try { json = JSON.parse(await readFile(file, 'utf8')); } catch { defects.push('vitest produced no JSON report: ' + out.slice(0, 300)); }
  if (json) {
