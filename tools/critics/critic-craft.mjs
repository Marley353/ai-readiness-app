...
export async function run(browser) {
  ...
  for (const scene of SCENES) {
    ...
    for (const o of objs) {
      ...
      if ((o.kitType === 'panel' || o.kitType === 'button' || o.kitType === 'header') && o.w > 0) { const on = (v) => { const m = Math.abs(Math.round(v) % 8); return m <= 1 || m >= 7; }; const W = VIEWPORTS['ipad-10'].width, H = VIEWPORTS['ipad-10'].height; const pnl = o.panel; const xOk = on(o.x) || on(W - (o.x + o.w)) || Math.abs(o.x - (W - o.w) / 2) <= 1 || (pnl && (on(o.x - pnl.x) || on(pnl.x + pnl.w - (o.x + o.w)))); const yOk = on(o.y) || on(H - (o.y + o.h)) || Math.abs(o.y - (H - o.h) / 2) <= 1 || (pnl && (on(o.y - pnl.y) || on(pnl.y + pnl.h - (o.y + o.h)))); if (!xOk || !yOk) defects.push(`${scene.name}: ${o.kitType} "${o.label}" at ${Math.round(o.x)},${Math.round(o.y)} off the 8pt grid (left/right/centre anchors)`); }
    }
